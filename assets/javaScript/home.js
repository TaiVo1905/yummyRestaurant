...
function renderFeaturedDishes(data){
    ...
    menuAllDish.innerHTML += `
                              <div class="menu_card">
                                  ...
                                  <h5>${dish.name}</h5>
                                  <h6>${dish.price}</h6>
                                  <button id="add_to_cart-${dish.id}"> Thêm vào giỏ hàng</button>
                              </div>
                              `;
    })
}
...
//     document.querySelector('#add_to_cart').forEach(button =>{
//         button.addEventListener('click', function(){
//             if (featuredDishes.name === )
...
//                 // food_Note: foodNote
//             };
//             const exitItem = allData.cart.find(item=> item. food_Name === foodItem.name && item.user_ID === userID);
//             // const exitdata = allData.cart.find()
//             if (exitItem == true){
...
//             }
            ...
            // const existingItemIndex = allData.carts.findIndex(item => item.food_Name === foodItem.name && item.user_ID === userID);
